...
if (isset($_GET['_id'])) {
    $category_id = $conn->real_escape_string($_GET['_id']);
    $product = $conn->query("SELECT p.product_photo, p.product_id, p.category_id, p.name, p.description, p.prize, p.stock, c.category_name FROM tbl_products p LEFT JOIN tbl_category c ON p.category_id = c.category_id WHERE p.stock > 0 AND p.category_id='{$category_id}'");
    if ($product->num_rows < 1) {
        header("Location: ../");
    }else{
       $category = $product->fetch_assoc();
...
<body>
    ...
    <div class="container-fluid bg-secondary mb-5">
        <div class="d-flex align-items-center justify-content-center" style="min-height: 300px">
            <h1 class="font-weight-semi-bold text-uppercase mb-3"><?= $category_name?></h1>
        </div>
    </div>
    <!-- Page Header End -->
